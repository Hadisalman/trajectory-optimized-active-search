...
caxis(clims); colormap(brighten(jet, .7));colormap hot
plot_patches(regions, 'rule'); hold on;

 [c,h] = contour( reshape(x_view(:,1), x_shape),  reshape(x_view(:,2), x_shape),  reshape(f_view, x_shape), linspace(clims(1), clims(2), 9),'ShowText','off');
...
if ~isempty(x_collected)
  scatter(x_collected(:,1), x_collected(:,2), 20, 'k', 'filled');
end
